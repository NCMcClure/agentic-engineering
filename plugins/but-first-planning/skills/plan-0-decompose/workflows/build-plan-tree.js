...
if (!PAGES) {
  ...
  const listing = await agent(
    `List every CONTENT page of the spec at ${SPEC}/ as paths relative to ${SPEC}/ (e.g. "02-runtime/event-loop.md"). Include section content files only: EXCLUDE every index.md, everything under reference/ (glossary, adr), prototypes/, assets/, scripts/, and .site/. Use ls/glob, not judgment. ${BRIEF}`,
    {
      ...
      items: {
        ...
        properties: {
          ...
          issues: {
            ...
            items: {
              type: 'object',
              required: ['num', 'slug', 'title', 'type', 'whatToBuild', 'userFacing', 'anchors', 'acceptanceCriteria', 'checkpoint', 'blockedBy'],
              properties: {
                ...
                userFacing: { type: 'string', description: "'yes — <the user-visible surface>' or 'no — internal'. Required judgment on every AFK/HITL issue; for REVIEW issues put 'no — internal' (the author omits the line)" },
...
const CRITICS = [
  { key: 'coverage', prompt: `Audit COVERAGE of the plan tree at ${PLANDIR}/ against the spec: (a) every one of these ${PAGES.length} spec pages must be an anchor of at least one issue — build the matrix by grepping the tree (grep -rl "spec/<page>" ${PLANDIR}); pay special attention to user-docs-plan.md (its anchor is the early AFK docs-skeleton issue — flag if that issue is missing) and repository-layout.md; (b) every AFK/HITL issue file must end its What to build with a link-free "**User-facing**: yes — <surface>" or "**User-facing**: no — internal" line (grep for issues missing it); (c) EVERY open question in this ledger must be addressed — a 'decision-issue' one has a dedicated HITL issue whose title names the decision, an 'inline-note' one is referenced in some issue's What to build; list every miss:\n` },
  { key: 'slices', prompt: `Audit SLICE QUALITY per ${SKILL}/VERTICAL-SLICES.md (read it first): sample at least 3 issues from EVERY sprint in ${PLANDIR}/ and flag horizontal slices (layer-only work with no observable behaviour), slices not verifiable alone, dishonest AFK labels (needs human judgment), oversized slices that should split, sprint goals that are not observable exits, and REVIEW gates at the wrong granularity (one per slice, missing where ${SPEC}/reference/adr/0002-ui-posture.md requires them, or present under the headless posture). Ledger for context:\n` },
  { key: 'testability', prompt: `Audit TESTABILITY: acceptance criteria must be executable/checkable targets (honour any acceptance-criteria/testing conventions the spec pins — check ${SPEC}/reference/adr/ for them), checkpoint commands must be plausible, decision issues must have decision-shaped (not code-shaped) criteria, and REVIEW issues must have observation-shaped criteria with a manual walkthrough checkpoint — never a fake command. Sample broadly across ${PLANDIR}/. Ledger for context:\n` },
]
