export const meta = {
  ...
  whenToUse: "The headless complement to plan-2-grill-spec's interactive grill — when the user asks for a deep/comprehensive/multi-agent review of the spec, or plan-2's autonomous mode. Args: {root, context?, extraLenses?, applyFixes?, rounds?, maxFindings?}. applyFixes=false (default) reports; true edits the spec pages. rounds (1-3, applyFixes only) re-reviews the changed files after fixing until a round confirms no critical/major finding. maxFindings (default 60) caps the verified set per round by severity.",
  phases: [
...
- Only when a finding requires a genuine product decision that is NOT derivable from
  ...
  as applied. (The interactive grill — plan-2-grill-spec — resolves these with the user.)
