export const meta = {
  ...
  whenToUse: "plan-6-edit's propagation step when the blast radius is wide (roughly 10+ affected issues) and the user has approved the affected-issue list. Args: {root, changedSpecFiles, changeSummary}. Re-cuts are flagged back to the skill, not applied.",
  phases: [
...
return {
  ...
  verify,
  note: recuts.length
    ? 're-cut verdicts were NOT applied — route them through plan-4-plan (split/merge/replace) and republish per SYNC.md'
    : undefined,
}
