...
| Label | Meaning |
|---|---|
| `epic::EE-<slug>` | Which epic the issue belongs to. Created **lazily by plan-1** when an epic's first sprint is published. One hue per epic. |
...
plan-1** when that sprint is published — never pre-create milestones for the
whole plan.
...
commands, and the known `glab` traps) lives in `plan-1-publish-issues`'s
