...
doesn't live-reload on every save). The plan-6-edit skill reads that file and
...
INSTRUCTIONS = (
    ...
    '"specFile". To act on them, run the plan-6-edit skill: for each unresolved '
    'comment, treat "specFile" as the changed spec file, route and propagate the '
    "edit per plan-6-edit, then set that comment's \"resolved\" field to true (do "
