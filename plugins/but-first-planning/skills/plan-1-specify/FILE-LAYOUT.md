...
`reference/` is reserved and scaffolded by `plan-0-init`. It holds `glossary.md` (the
domain vocabulary) and `adr/` (decision records). Those are maintained by
`plan-2-grill-spec` and `plan-3-architect-spec`, not hand-edited here — but link into
