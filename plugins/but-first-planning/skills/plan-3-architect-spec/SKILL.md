...
# plan-3-architect-spec — get the architecture right before code
...
posture as `plan-2-grill-spec`: one question at a time, recommended answers). Walk the
...
- **Naming a deepened module after a concept not in the glossary?** Add the term — same discipline as `plan-2-grill-spec` ([its CONTEXT-FORMAT.md](../plan-2-grill-spec/CONTEXT-FORMAT.md)).
- **Sharpening a fuzzy term?** Update the glossary right there.
- **A load-bearing decision, or the user rejecting a candidate for a reason future reviews must respect?** Offer an ADR ([its ADR-FORMAT.md](../plan-2-grill-spec/ADR-FORMAT.md)) — framed as "want me to record this so future architecture reviews don't re-suggest it?" Only when it clears the bar.
...
```
Workflow({
  scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/plan-3-architect-spec/workflows/deepening-review.js",
  args: {
    root: "<absolute repo root>",
    skillDir: "${CLAUDE_PLUGIN_ROOT}/skills/plan-3-architect-spec",
...
When the design is sharper, point the user back at `plan-2-grill-spec` if new
ambiguity surfaced, or forward to `plan-4-plan` to decompose the now-solid design
