...
if (!A || !A.root || !A.skillDir) {
  throw new Error('args must be an object: {root: <absolute repo root>, skillDir: <plan-4-plan skill dir>, pages?, context?, lenses?}')
}
...
await agent(
  `You are the plan-index author. ${PLANDIR}/index.md currently holds a plan-0-init stub. Rewrite its "## Epics" section as the epic table per ${SKILL}/PLAN-FORMAT.md (READ IT): one row per epic dir on disk under ${PLANDIR}/ (link (NN-slug/epic.md)), with real sprint/issue counts from disk, Status not-started, and a Total row. Keep any non-Epics prose that still makes sense. Then run: python3 ${PLANDIR}/verify-plan-tree.py — report its exit code and first errors verbatim. Fix nothing else.`,
  {
