...
that's a signal to grill the spec (`plan-2-grill-spec`) until it can be AFK — the
...
- A slice should be completable in one focused sitting. If it sprawls, split it along the next natural seam (often the seams `plan-3-architect-spec` already named).
