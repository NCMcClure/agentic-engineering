...
# plan-4-plan — turn the spec into an executable backlog
...
design, do that first (`plan-2-grill-spec`, `plan-3-architect-spec`) — re-cutting issues
...
natural seams (often the architecture work from `plan-3-architect-spec` already
...
```
.plan/plan/
├── index.md                         # epic table (update the plan-0-init stub)
├── 01-<epic-slug>/
...
`not-started` — publishing (`plan-5-publish-issues`) fills in real references later.
...
Next steps: `plan-5-publish-issues` to push a sprint's issues to the tracker (do this
lazily, sprint by sprint, not all at once), or `build-next-issue` once work
is under way. If revising the spec later, `plan-6-edit` keeps the plan in sync.
...
```
Workflow({
  scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/plan-4-plan/workflows/build-plan-tree.js",
  args: {
    root: "<absolute repo root>",
    skillDir: "${CLAUDE_PLUGIN_ROOT}/skills/plan-4-plan",
...
- **Spec anchors** make the plan auditable against the design and let `plan-6-edit` find which issues a spec change affects.
