...
truth, and lets `plan-6-edit` find exactly which issues a spec change touches.
...
- If a slice has no spec anchor, that's a red flag: either the spec is missing the section (go back to `plan-1-specify`), or the slice isn't actually realising the design (reconsider it).

## Anchors enable change propagation

Because anchors are concrete file paths, `plan-6-edit` can answer "I changed
...
and kept current. When `plan-6-edit` moves or renames a spec file, it updates the
