...
Lives at `.plan/plan/publish-issues.py` (copied there by plan-0-init; bundled
with plan-5-publish-issues). Deterministic and idempotent: it parses the plan
...
plan-6-edit propagation helper). Stdlib only; failures per issue are collected,
...
def tracker() -> dict:
    ...
    if not TRACKER_MD.is_file():
        die(f"{TRACKER_MD} not found — run plan-0-init first")
