...
# plan-5-publish-issues — push a sprint to the tracker
...
`.plan/tracker.md` (written by `plan-0-init`) says where issues go and how. Read it
...
`.plan/plan/` by `plan-0-init` — backfill it from `assets/` if an older workspace
...
- It does not change the plan's structure or acceptance criteria — if those need to change, that's `plan-6-edit`, not a publish.
