...
files they realise (see `plan-0-decompose`'s SPEC-ANCHORS reference). Because anchors are
...
| The slice is now too big / too small | Re-cut via `plan-0-decompose` (split, merge, or replace), preserving anchors |
...
`plan-1-publish-issues` reference (TRACKER-GITHUB.md / TRACKER-GITLAB.md); the bundled
...
dependency order (blockers first), the same order `plan-1-publish-issues` creates them in.
