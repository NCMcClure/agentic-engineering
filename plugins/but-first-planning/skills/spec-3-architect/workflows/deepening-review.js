export const meta = {
  ...
  whenToUse: "spec-3-architect's autonomous mode, or a broad spec (roughly 10+ sections) where a single Explore pass would go shallow; the interactive report+grilling loop stays the default. Args: {root, skillDir, context?, extraLenses?, apply?}. apply 'none' (default) reports only; 'strong' also applies the confirmed Strong candidates as spec edits with ADRs.",
  phases: [
...
if (!A || !A.root || !A.skillDir) throw new Error("args must be an object: {root: <absolute repo root>, skillDir: <spec-3-architect skill dir>, context?, extraLenses?, apply?: 'none'|'strong'}")
...
const LENSES = [
  ...
  { key: 'repo-layout', prompt: `You are the repo-layout lens. Read the spec's repository-layout.md (find it by exact filename; if it doesn't exist, report that absence as your single finding). Pressure-test the drawn source tree for locality and progressive disclosure per the skill's ../spec-1-specify/CODEBASE-LAYOUT.md: does each spec module get exactly one directory (no concern smeared across directories, no directory owning two concerns)? Do the AGENTS.md hub rules hold on the drawn tree — hubs at every non-leaf code level, no source files beside a hub, leaf directories one-line-describable by their parent? Would a typical change per the spec's flows stay inside one module directory? Each candidate proposes the layout change and names the spec files it re-draws.` },
